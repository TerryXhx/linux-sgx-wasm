...
static char global_heap_buf[20 * 1024 * 1024] = { 0 };
#else
...
{   
    ...
    sgx_wasm_t *wasm_hdr = (sgx_wasm_t*)sgx_get_wasm_sec_buf_addr();
    uint8_t *wasm_file_buf = wasm_hdr->wasm_blob;
    uint32_t wasm_file_size = wasm_hdr->size;
    ...
    enclave_module->wasm_file = (uint8 *)enclave_module + sizeof(EnclaveModule);
    bh_memcpy_s(enclave_module->wasm_file, wasm_file_size, wasm_file_buf,
                wasm_file_size);
